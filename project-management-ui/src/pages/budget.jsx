  ...
  return (
    <div className="bg-black text-white min-h-screen px-6 py-16 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Overall Summary */}
        {budgets.length > 0 && (
          <div className="bg-zinc-800/60 p-6 mt-16 rounded-xl border border-white/10 text-sm text-gray-300 shadow-subtle">
            <h3 className="text-lg font-semibold text-white mb-2">Total Summary</h3>
            <p>Total Budget: ${budgets.reduce((sum, b) => sum + b.totalBudget, 0).toFixed(2)}</p>
            <p>Actual Cost: ${budgets.reduce((sum, b) => sum + b.actualCost, 0).toFixed(2)}</p>
            <p>Forecast Cost: ${budgets.reduce((sum, b) => sum + b.forecastCost, 0).toFixed(2)}</p>
          </div>
        )}
      </div>
    </div>
